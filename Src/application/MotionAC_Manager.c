...
void MotionAC_manager_init(MAC_enable_lib_t enable)
{
	...
	MotionAC_GetKnobs(pKnobs);
	pKnobs->MoveThresh_g = MOVE_THR_G;
	pKnobs->Run6PointCal = (uint8_t)CalibrationMode;
	pKnobs->Sample_ms = ReportInterval;
	(void)MotionAC_SetKnobs(pKnobs);
}
 ...
 * @retval none
 */
void MotionAC_manager_compensate(SensorAxes_t *data_in, SensorAxes_t *data_out)
{
  ...
  ans_int32[0] = data_in->AXIS_X - acc_bias_to_mg(acc_calib.AccBias[0]);
  ans_int32[1] = data_in->AXIS_Y - acc_bias_to_mg(acc_calib.AccBias[1]);
  ans_int32[2] = data_in->AXIS_Z - acc_bias_to_mg(acc_calib.AccBias[2]);
  ...
  data_out->AXIS_X = (int32_t)ans_float[0];
  data_out->AXIS_Y = (int32_t)ans_float[1];
  data_out->AXIS_Z = (int32_t)ans_float[2];
}
