...
const uint8_t SW_Version[4] ="2300";
...
const uint32_t Ser_Number = 1;
	...
	float32_t Rs_CO;					//In ohm
	float32_t Rs_NO2;					//In ohm
	float32_t Rs_NH3;					//In ohm
#endif
#if (GUI_SUPPORT==1)
#if (IMU_PRESENT==1)
...
//HR and UVx values are displayed in the same page of pressure value (Environmental Page);
...
//the presence of the humidity, UVx and Analog Module sensors
	...
	BIT_SET(SensorStatusReg,22);	//Set IMU presence in SensorStatusRegister
//	NumberOfDevices++;				//Increment number of sensors mounted
	...
	if (LSM9DS1_status == (uint8_t)LSM9DS1_OK)
	{
		...
		BIT_SET(SensorStatusReg,6);	//Set IMU status in SensorStatusRegister
	}
	/* MotionFX_Init() */
...
#if (GUI_SUPPORT==0)				//Defined in main.h
...
//	Humidity_Sensor_GetData(Sensor_Hub_1_out, Humidity_percent_1_data);
	...
	Display_Update(Mux_Int_3_out, &display_info_list[3]);
   	...
   	if (send_lcl_voc_data || send_lcl_uvx_data || send_lcl_hum_data || send_lcl_prs_data || send_lcl_pms_data)
...
//	const double_t g = 9.80665;				//g is the acceleration of gravity
	...
	T_Out = PressTemp->Tout + T_Correction;
...
#if (HUMIDITY_SENSOR_PRESENT==0)	//Take the temperature value from the pressure sensor
...
void Humidity_Sensor_Handler(HTS221_MeasureTypeDef_st *HumTemp, uint8_t* Buff)
{
	...
	static double p_TempD = 0;
	static uint8_t p_Hum = 0;
	static float32_t TemperatureP = 0;

	//When the RH*10 reaches 900 (90%) then it applies the correction in proportion
	//to the 1000 - RH*10 difference. This prevents RH% from exceeding 100%
	if (HumTemp->Hout > 890)
		RH_Correction = (int32_t)(lrintf(((float32_t)((1000 - HumTemp->Hout) * ((RH_Correction)/100)))));
...
//	T2_Out = (uint16_t)(lrintf((float)(SI*10)));	//Use HI as T2_Out
	...
	if ((Humidity > 99) && (!ServiceTimer2.Start) && (!ServiceTimer3.Start))
	{
...
{
	static const uint32_t AverageWindow = 17280;	//3600*24/5: Number of readings in 24 hour
...
//	MC_10p0_f = (uint16_t)(Particulate->mc_10p0);
	...
	mc_4p0_avg = approxMovingAverage(mc_4p0_avg, MC_4p0_f, AverageWindow, CorrectionFactor);
	Particulate->mc_4p0_mean = mc_4p0_avg;
	mc_10p0_avg = approxMovingAverage(mc_10p0_avg, MC_10p0_f, AverageWindow, CorrectionFactor);
...
void Gas_Sensor_Handler(ANLG_MeasureTypeDef_st *anlg, uint8_t* Buff)
{
	...
	const float32_t CorrectionFactor_24h = (1+(1/AverageWindow_24h));
...
#if (FULL_MODE==1)
	no2_1h_avg = approxMovingAverage(no2_1h_avg, anlg->NO2, AverageWindow_1h, CorrectionFactor_1h);
...
#if (PRESSURE_SENSOR_PRESENT==0)			//Defined in main.h
