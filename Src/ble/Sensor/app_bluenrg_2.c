...
uint8_t bdaddr[BDADDR_SIZE];
	...
	extern __IO uint8_t send_ecomp;
	extern __IO uint8_t send_ar;
#endif
...
/* USER CODE BEGIN PFP */
#if ((USE_SENSUS191==1) && (IMU_PRESENT==1))
	static void Set_Motion_Values(void);
	static void Set_ECompass_Values(void);
	static void Set_MotionAR_Values(void);
	static bool Send_Mot = false;
	static bool Send_Quat = false;
	static bool Send_EComp = false;
	static bool Send_AR = false;
#endif
#endif	//BLE_SUPPORT==1
/* USER CODE END PFP */
...
//ret = aci_gap_clear_security_db();
  ...
  {
     PRINT_DBG("Error while adding BlueNRG2 SW service: 0x%02x\r\n", ret);
#if (USE_SENSUS191==0)
    while(1);
#endif
  }
  ...
  if(ret == BLE_STATUS_SUCCESS)
  {
     PRINT_DBG("BlueNRG2 Config service added successfully.\r\n");
  } else
  {
     PRINT_DBG("Error while adding BlueNRG2 Config service: 0x%02x\r\n", ret);
     while(1);
  }
  ...
  if(ret == BLE_STATUS_SUCCESS)
  {
     PRINT_DBG("Time service added successfully.\r\n");
  } else
  {
     PRINT_DBG("Error while adding Time service: 0x%02x\r\n", ret);
     while(1);
  }
...
#if (IMU_PRESENT==1)
		if ((send_mot) || (send_quat) || (send_ecomp) || (send_ar))
		{
		/* Update Acceleration, Gyroscope and Sensor Fusion data */
	#if (USE_SENSUS191==1)
			if (send_lcl_imu_data_to_ble)
			{
				Set_Motion_Values();
				Set_ECompass_Values();
				Set_MotionAR_Values();
				send_lcl_imu_data_to_ble = false;
			}
	#else
			Set_Random_Motion_Values(counter);
	#endif	//USE_SENSUS191==1
			if ((send_mot) && (Send_Mot))
			{
				Acc_Update(&x_axes, &g_axes, &m_axes);
				Send_Mot = false;
			}
			if ((send_quat) && (Send_Quat))
			{
				Quat_Update(&q_axes);
				Send_Quat = false;
			}
			if ((send_ecomp) && (Send_EComp))
			{
...
#if (IMU_PRESENT==1)
/**
 * @brief  Set values for all motion sensor data
 *
 * @param  None
 * @retval None
 */
static void Set_Motion_Values(void)
{
	(void)memcpy((void *)&x_axes, (void *)&(IMU_Values.Acc_Out), 3U * sizeof(int32_t));
	(void)memcpy((void *)&g_axes, (void *)&(IMU_Values.Gy_Out), 3U * sizeof(int32_t));
	(void)memcpy((void *)&m_axes, (void *)&(IMU_Values.Mag_Out), 3U * sizeof(int32_t));
	...
	if(Quaternions_9X_1_data[3] < 0)
	{
		q_axes.AXIS_X = (int32_t)(Quaternions_9X_1_data[0] * (-10000));
		q_axes.AXIS_Y = (int32_t)(Quaternions_9X_1_data[1] * (-10000));
		q_axes.AXIS_Z = (int32_t)(Quaternions_9X_1_data[2] * (-10000));
	} else
	{
		q_axes.AXIS_X = (int32_t)(Quaternions_9X_1_data[0] * 10000);
		q_axes.AXIS_Y = (int32_t)(Quaternions_9X_1_data[1] * 10000);
...
/**
 * @brief  Set values for ECompass sensor data
 *
 * @param  None
 * @retval None
 */
static void Set_ECompass_Values(void)
{
	Angle = (uint16_t)trunc(100*Heading_9X_1_data[0]);
	Send_EComp = true;
}

/**
 * @brief  Set values for MotionAR sensor fusion data
 *
 * @param  None
 * @retval None
 */
static void Set_MotionAR_Values(void)
{
	Activity = (uint8_t)Activity_Index_1_data[0];
	Send_AR = true;
}
#endif	//IMU_PRESENT==1
#else	//USE_SENSUS191==0
/**
