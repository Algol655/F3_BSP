...
uint8_t CCS811_1[1] = 	{							//Added By Me!!!
							CSS811_MEAS_MODE_VAL
						};
uint8_t CCS811_2[1] = 	{							//Added By Me!!!
							CSS811_APP_START_VAL
						};
...
{
	CCS811_Error_et ret;
	ret = CCS811_ReadReg(B_Addr, CSS811_HW_ID, 1, buff);
...
{
  CCS811_Error_et ret;
  ret = CCS811_ReadReg(B_Addr, CSS811_STATUS, 1, buff);
...
{
  CCS811_Error_et ret;
  ret = CCS811_ReadReg(B_Addr, CSS811_ERROR_ID, 1, buff);
...
//	uint8_t  *modeset;
	...
	*modeset = mosetting;				//"modeset" is used uninitialized in this function warning
	...
	if (CCS811_ReadReg(B_Addr, CSS811_MEAS_MODE, 1, &mosetting))	//Read what's currently there
		return CCS811_ERROR;
	HAL_Delay(i2c_delay);
	mosetting &=~(7<<4);					//Clear DRIVE_MODE bits
	mosetting |= (mode << 4);				//Mask in mode

	if (CCS811_WriteReg(B_Addr, CSS811_MEAS_MODE, 1, &mosetting))
		return CCS811_ERROR;
...
CCS811_Error_et	CCS811_Get_Measurement(CCS811_MeasureTypeDef_st *Measurement_Value)
{
	...
	ret = CCS811_ReadReg(CCS811_BADDR, CSS811_ALG_RESULT_DATA, 8, &data_rq[0]);

	/*	TVOC value, in parts per billion (ppb)
	eC02 value, in parts per million (ppm) */
	ECO2 = ((uint8_t)data_rq[0] << 8) | data_rq[1];
	eCO2_MovingAverage(&ECO2, &(Measurement_Value->eCO2), 10);
	ETVOC = ((uint8_t)data_rq[2] << 8) | data_rq[3];
...
//	Measurement_Value->eTVOC = ((uint8_t)data_rq[2] << 8) | data_rq[3];
	...
	Measurement_Value->RawData = ((uint8_t)data_rq[6] << 8) | data_rq[7];
...
/*	uint8_t env[6];		//Debug
	...
	ret = CCS811_WriteReg(CCS811_BADDR, CSS811_ENV_DATA, 4, &envData[0]);
...
CCS811_Error_et	CCS811_SoftRST()
{
	...
	ret = CCS811_WriteReg(CCS811_BADDR, CSS811_SW_RESET, 4, &rstCMD[0]);
...
CCS811_Error_et	CCS811_Save_Baseline(bool SaveToFlash)
{
	...
	ret = CCS811_ReadReg(CCS811_BADDR, CSS811_BASELINE, 2, &baseline[0]);
...
CCS811_Error_et	CCS811_Restore_Baseline(bool RestoreFromFlash)
{
	...
	ret = CCS811_WriteReg(CCS811_BADDR, CSS811_BASELINE, 2, &res_bs[0]);
...
CCS811_Error_et MX_CCS811_Init()
{
	...
	if (Status & 0x80)				//If FW is in application mode
	{
		...
		if (CCS811_Init(CCS811_BADDR, CSS811_MEAS_MODE, &CCS811_1[0], 1))
			return CCS811_ERROR;
	    Sleep(50);
//		CCS811_ReadReg(CCS811_BADDR, CSS811_MEAS_MODE, 1, &tmp);					//Test
	} else return CCS811_ERROR;
